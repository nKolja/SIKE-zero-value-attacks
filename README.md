...
This library is built upon the official SIKE implementation v 3.4 (see below).
...
- `side_channel.c` contains the top level functions used to execute a side-channel attack. There are six functions, three used to attack Bob, and three symmetric functions for attacking Alice. Function baseline\_* computes a pair of public keys, such that the computing party computes an invertion of 0 with one key, and invertion of a random value with the other key. Function malicious\_* uses as input index k of the bit/trit we are trying to extract, and first k bits/trits (0,...,k-1) of the attacked party. It outputs one/two public key/s where the attacked party will do an inversion of zero with said public key/s depending on the value of their secret keys k'th bit/trit. Function *\_computation is used to simulate Cortex-M4 computations on a 64-bit machine for testing purposes.
