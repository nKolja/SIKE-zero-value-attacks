...
*
* Creates malicious public keys of Alice which extract k+1'st bit of Bob's secret key.
* The public key created is to be sent to Bob, and computed by Bob.
* Input requires "k" the index of bit we are trying to extract, and the known first k bits of Bob  (indexed by 0,1,...,k-1)
*********************************************************************************************/ 
#include <stdio.h>
#include <string.h>
...
int main(int argc, char* argv[])
{
    ...
    Status = malicious_alice(argc, argv);             // Alice key generation 
    if (Status != PASSED) {
        printf("\n\n   Error detected! \n\n");
        return FAILED;
    }
