...
*
* Creates malicious public keys of Bob which extract k+1'st trit of Alice's secret key.
* The public keys created are to be sent to Alice, and computed by Alice.
* Input requires "k" the index of trit we are trying to extract, and the known first k trits of Alice (indexed by 0,1,...,k-1)
*********************************************************************************************/ 
#include <stdio.h>
#include <string.h>
...
int main(int argc, char* argv[])
{
    ...
    Status = malicious_bob(argc, argv);             // Alice key generation 
    if (Status != PASSED) {
        printf("\n\n   Error detected! \n\n");
        return FAILED;
    }
